...
function userHeader(from) {
  const uname = from.username ? `@${from.username}` : "(tanpa username)";
  return `👤 ${from.first_name || ""} ${uname} — ID: ${from.id}`;
}
...
function registerRelay(bot, store, config) {
  ...
  bot.on("message", async (ctx, next) => {
    ...
    if (!isSessionOpen(store, ctx.from.id)) {
    ...
    // actual message content beneath it.
    const header = await ctx.telegram.sendMessage(
      config.ADMIN_GROUP_ID,
      userHeader(ctx.from),
      endSessionKeyboard(ctx.from.id)
    );
